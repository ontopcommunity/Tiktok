...
function openVideoModal(index) {
    ...
    document.getElementById('modal-video-info').innerHTML = `
        ...
        <div class="flex items-center gap-3 bg-slate-900/40 p-3 rounded-xl border border-white/5 mb-5 shadow-inner cursor-pointer hover:border-purple-500/50 hover:bg-slate-800/80 transition group" onclick="scanMusicFromModal('${d.music.playUrl}', '${safeMusicTitle}')" title="Bấm để truy vết các video sử dụng nhạc này">
...
async function fetchUserInfo(isLoadMore = false) {
    ...
    try {
        ...
        if (!isLoadMore && data.author) {
            fullUserData = data; 
            renderUserInfoExpanded(); // LUÔN BUNG HỒ SƠ 
        }
